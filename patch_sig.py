...
def TLVGenerator(rampatch_list, output_fname, rsp_config, signed_config):
	...
	try:
		fout_hash_name = output_fname[:-4] + '.hash'
		with open(fout_hash_name, "w+b") as fout_hash:
			# write #
			hash = SHA256.new()
			hash.update(fout_buf)
			fout_hash.write(hash.hexdigest())
			#fout_hash.write(binascii.a2b_hex(hash.hexdigest()))
			print(hash.hexdigest())
			fout_hash.close()
	except IOError:
		print "Failed to write " + fout_hash_name
		exit()
	...
	if signed_config == 3:
		...
		HASH = ''
		SIGNATURE = getSignature(fout_buf)
		PUB_KEY = getRSAData(PUB_KEY_FILE)
		new_str = ""
		for i in SIGNATURE:
			new_str += "%s " % (i.encode('hex'))
		print new_str
